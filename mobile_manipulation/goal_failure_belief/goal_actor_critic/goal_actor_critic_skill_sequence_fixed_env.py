...
class TrainGoalActorCriticFixedEnv:
    ...
    def __init__(self, composite_task_config, skill_config, seed=0, goal_dist_coef=0.001, gpu_id=0) -> None:
        ...
        self.num_envs = 1
        self.checkpoint_dir = os.path.join(parent_dir, f'{self._prefix}-models-{seed}-fixed-env')
        self.tensorboard_dir = os.path.join(parent_dir, f'{self._prefix}-tensorboard-{seed}-fixed-env')
        self.seed = int(seed)
    ...
    def _init_policies(self, auto_reset_done=False):
        self._base_policies = [CompositeSkill(self._composite_task_config.SOLUTION, self.envs[0])]
    ...
    def _init_train(self):
        ...
        self.envs = [env]
        env.seed(self._skill_config.TASK_CONFIG.SEED)
        ...
        self.episode_rewards = torch.zeros(self.num_envs, 1)
    ...
    def _init_rollouts(self):
        ...
        for i in range(self._num_reset):
            self.envs[0].reset()
        observations = [self.envs[0].reset()]
    ...
    def modify_env_goal(self, goal_residual):
        out = self.envs[0].set_goal(goal_residual=goal_residual.cpu().numpy(), current_obs=self._last_obs[0])
        self._last_obs[0] = out
    ...
    def step(self):
        ...
        with self.timer.timeit("sample_goal"):
            ...
            self.modify_env_goal(output_batch["action"][0])
        ...
        with self.timer.timeit("sample_action"):
            ...
            action = self._base_policies[0].act(self._last_obs[0])

        with self.timer.timeit("step_env"):
            obs, rews, done, info = self.envs[0].step(action)
            self.num_steps_done += n_envs

        with self.timer.timeit("update_info"):
            self._last_obs[0] = obs
            obs["place_goal"] = obs["place_goal_at_base"]
            obs["pick_goal"] = obs["pick_goal_at_base"]
            stage_success = info["stage_success"]
            stage_successes = DENSE_TASK_REWARD * np.sum([stage_success[stage] for stage in stage_success])
        ...
        with self.timer.timeit("update_current_skill"):
            updated_obs = obs
            original_sensors = list(obs.keys())
            for key in original_sensors:
                if "rgb" in key:
                    del updated_obs[key]
            updated_obs["next_skill"] = np.zeros(len(self._skill_sequence))
            updated_obs["next_skill"][self._base_policies[0]._skill_idx] = 1
            reset_env = self._base_policies[0].check_if_done(obs) and (self._base_policies[0]._skill_idx == len(self._skill_sequence) - 1)
            if self._base_policies[0]._skill_idx > 0:
                current_skill_name = self._base_policies[0].skill_sequence[self._base_policies[0]._skill_idx]
                if (current_skill_name == "NextTarget") and (self._stage_successes == 0):
                    reset_env = True
            if reset_env:
                done = True
        # -------------------------------------------------------------------------- #
        ...
        if done:
            ...
            for i in range(self.num_envs):
                self._base_policies[i].to(self.device)
            is_truncated = info.get(
                "is_episode_truncated", False
            )

            # Estimate values of actual next obs
            if is_truncated and not ignore_truncated:
                are_truncated = [True]
            ...
            self._last_obs[0] = ob
            self._base_policies[0].reset(ob)
            updated_obs = ob
            ob["place_goal"] = self.envs[0].get_original_place_goal()
            ob["pick_goal"] = self.envs[0].get_original_pick_goal()

            updated_obs["next_skill"] = np.zeros(len(self._skill_sequence))
            updated_obs["next_skill"][self._base_policies[0]._skill_idx] = 1
            if (self._stage_successes == DENSE_TASK_REWARD * len(info["stage_success"])):
                diff_stage_successes += FINAL_TASK_REWARD
            self._stage_successes = 0
        ...
        with self.timer.timeit("batch_obs"):
            ...
            done_masks = torch.tensor([done], dtype=torch.bool).unsqueeze(1)
        ...
        with self.timer.timeit("update_stats"):
            self.episode_rewards += diff_stage_successes
            if done:
                episode_info = self._extract_scalars_from_info(
                    info
                )
                episode_info["return"] = self.episode_rewards.item()
