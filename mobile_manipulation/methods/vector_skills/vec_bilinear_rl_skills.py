...
class BilinearRLSkill(Skill):
    ...
    def act(self, obs, **kwargs):
        with torch.no_grad():
            ...
            outputs = self.actor_critic.act(step_batch, deterministic=True, include_entropy=False)
...
class BilinearPickRLSkill(BilinearRLSkill):
    def act(self, obs, **kwargs):
    ...
    def check_success_from_info(self):
        self._vec_rl_env.async_call_at(self._vec_rl_env_idx, "get_metrics", {})
        info = self._vec_rl_env.wait_step_at(self._vec_rl_env_idx)
...
@my_registry.register_skill
class BilinearNavRLSkill(BilinearRLSkill):
    def reset(self, obs, **kwargs):
        super().reset(obs, **kwargs)
        self._vec_rl_env.async_call_at(self._vec_rl_env_idx, "set_task_is_stop_called", {"action": self._config.ACTION, "is_stop_called": False})
        self._vec_rl_env.wait_step_at(self._vec_rl_env_idx)
    ...
    def check_success_gt(self):
        self._vec_rl_env.async_call_at(self._vec_rl_env_idx, "get_info", {})
        info = self._vec_rl_env.wait_step_at(self._vec_rl_env_idx)
        success = info["rearrange_nav_success"]
        return success

    def check_stop_called(self):
        self._vec_rl_env.async_call_at(self._vec_rl_env_idx, "get_task_is_stop_called", {"action": self._config.ACTION})
        task_action_is_stop_called = self._vec_rl_env.wait_step_at(self._vec_rl_env_idx)
        return task_action_is_stop_called


@my_registry.register_skill
class BilinearPlaceRLSkill(BilinearRLSkill):
    def act(self, obs, **kwargs):
        step_action = super().act(obs, **kwargs)
    ...
    def should_terminate(self, obs, **kwargs):
        if self.is_timeout():
            ...
            if is_grasped:
                ...
                self._vec_rl_env.async_call_at(self._vec_rl_env_idx, "grip_desnap", {"should_desnap": False})
                self._vec_rl_env.wait_step_at(self._vec_rl_env_idx)
            return True
...
@my_registry.register_skill
class BilinearSetMarkerRLSkill(BilinearRLSkill):
    def reset(self, obs, **kwargs):
        self._has_set = False
...
@my_registry.register_skill
class BilinearPickPlaceRLSkill(BilinearRLSkill):
    def reset(self, obs, **kwargs):
        return super().reset(obs, **kwargs)
