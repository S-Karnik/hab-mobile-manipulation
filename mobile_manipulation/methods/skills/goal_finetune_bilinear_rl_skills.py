...
class GoalFTBilinearRLSkill(Skill):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.device = None
        self.action_shape = self._action_space[self._config.ACTION].shape
        self.skill_index = self._config.get("SKILL_INDEX", "")
        self.goal_finetune_ckpt_path = self._config.get("GOAL_CKPT_PATH", "")
        self.goal_action_space = spaces.Box(-np.inf, np.inf, (3,), dtype=np.float32)
        self.goal_action_shape = self.goal_action_space.shape
        self._is_goal_finetuned = False
        self._init_policy(self._config.CKPT_PATH, self.goal_finetune_ckpt_path, self.skill_index)

    def _init_policy(self, base_ckpt_path: str, goal_finetune_ckpt_path: str, skill_idx: str = ""):
        ckpt_dict = torch.load(base_ckpt_path, map_location="cpu")
        print(f"Loaded checkpoint from {base_ckpt_path}")
        ...
        actor_critic: BilinearActorCritic = policy.from_config(
            policy_config, self._obs_space, action_space, None
        )
        ...
        if goal_finetune_ckpt_path != "":
            ckpt_dict = torch.load(goal_finetune_ckpt_path, map_location="cpu")
            print(f"Loaded checkpoint from {goal_finetune_ckpt_path}")
            goal_actor_critic: BilinearActorCritic = policy.from_config(
                policy_config, self._obs_space, self.goal_action_space, "gaussian"
            )
            state_dict = ckpt_dict["state_dict" + skill_idx]
            state_dict = get_state_dict_by_prefix(state_dict, "actor_critic.")
            goal_actor_critic.load_state_dict(state_dict)
            goal_actor_critic.eval()

            self.goal_actor_critic = goal_actor_critic
            self._ckpt_config = ckpt_config
            self._is_goal_finetuned = True
    ...
    def reset(self, obs, **kwargs):
        ...
        self._goal_buffer = dict(
            ...
            prev_actions=torch.zeros(1, *self.goal_action_shape),
            masks=torch.zeros(1, 1, dtype=torch.bool),
        )
        self._goal_buffer = {k: v.to(self.device) for k, v in self._goal_buffer.items()}
    ...
    def modify_single_env_goal(self, goal_residual, observation):
        ...
        last_obs = self._rl_env.set_goal_per_skill(goal_residual=goal_residual, current_obs = new_observation, is_grasped=False)
        return last_obs
    ...
    def act(self, obs, **kwargs):
        with torch.no_grad():
            ...
            if self._is_goal_finetuned:
                goal_step_batch = dict(observations=batch, **self._goal_buffer)
                goal_outputs = self.goal_actor_critic.act(goal_step_batch, deterministic=True)
                new_obs = self.modify_single_env_goal(goal_outputs["action"][0].cpu().numpy(), obs)
                for sensor, value in new_obs.items():
                    step_batch["observations"][sensor][0] = torch.from_numpy(value).to(self.device)
            outputs = self.actor_critic.act(step_batch, deterministic=True)
            action = outputs["action"]
            value = outputs["value"]
            self._buffer.update(
                goal_dep_recurrent_hidden_states=outputs["goal_dep_rnn_hidden_states"],
                goal_ind_recurrent_hidden_states=outputs["goal_ind_rnn_hidden_states"],
                prev_actions=outputs["action"],
                masks=torch.ones(1, 1, device=self.device, dtype=torch.bool),
            )
            if self._is_goal_finetuned:
                self._goal_buffer.update(
                goal_dep_recurrent_hidden_states=goal_outputs["goal_dep_rnn_hidden_states"],
                goal_ind_recurrent_hidden_states=goal_outputs["goal_ind_rnn_hidden_states"],
                prev_actions=goal_outputs["action"],
                masks=torch.ones(1, 1, device=self.device, dtype=torch.bool),
            )
    ...
    def to(self, device=None):
        ...
        if self._is_goal_finetuned:
            self.goal_actor_critic.to(device)
        return self


@my_registry.register_skill
class GoalFTBilinearPickRLSkill(GoalFTBilinearRLSkill):
    def act(self, obs, **kwargs):
...
@my_registry.register_skill
class GoalFTBilinearNavRLSkill(GoalFTBilinearRLSkill):
    def reset(self, obs, **kwargs):
        super().reset(obs, **kwargs)
    ...
    def should_terminate(self, obs, **kwargs):
        ...
        end_type = self._config.END_TYPE
        if end_type == "gt":
            return self.check_success_gt(obs)
    ...
    def check_success_gt(self, obs):
        info = self._rl_env.get_info(obs)
...
@my_registry.register_skill
class GoalFTBilinearPlaceRLSkill(GoalFTBilinearRLSkill):
    def act(self, obs, **kwargs):
        step_action = super().act(obs, **kwargs)
...
@my_registry.register_skill
class GoalFTBilinearSetMarkerRLSkill(GoalFTBilinearRLSkill):
    def reset(self, obs, **kwargs):
        self._has_set = False
...
@my_registry.register_skill
class GoalFTBilinearPickPlaceRLSkill(GoalFTBilinearRLSkill):
    def reset(self, obs, **kwargs):
        return super().reset(obs, **kwargs)
