...
@baseline_registry.register_trainer(name="bilinear-ppo-v0")
class BilinearPPOTrainerV0(BaseTrainer):
    ...
    def __init__(self, config: Config):
        ...
        self.tensorboard_dir = self.config.TENSORBOARD_DIR
        self.checkpoint_folder = os.path.join(os.path.split(self.config.CHECKPOINT_FOLDER)[0], 'alt_checkpoints_normal_999_10_rew_no_rest_no_accum_force_2')
        self.tensorboard_dir = os.path.join(os.path.split(self.config.TENSORBOARD_DIR)[0], 'alt_tb_normal_999_10_rew_with_rest_cd_no_accum_force_2')
        # self.checkpoint_folder = os.path.join(os.path.split(self.config.CHECKPOINT_FOLDER)[0], 'alt_checkpoints_normal_3')
        # self.tensorboard_dir = os.path.join(os.path.split(self.config.TENSORBOARD_DIR)[0], 'alt_tb_normal_3')
    ...
    def save(self, ckpt_id):
        ...
        ckpt_path = os.path.join(
            self.checkpoint_folder, f"ckpt.{ckpt_id}.pth"
        )
    ...
    def resume(self):
        if not self.checkpoint_folder:
            return
        ckpt_path = get_latest_checkpoint(self.checkpoint_folder, False)
    ...
    def _place_reorient(self, env):
        sim = env.sim
        robot = sim.robot
        target_position = np.asarray(sim.pyb_robot.ee_state[4])
        sim.pyb_robot.set_joint_states(sim.robot.params.arm_init_params)
        ...
        target_orientation = transformations.quaternion_from_euler(*euler_angles)
        target_orientation = mn.Quaternion(((target_orientation[0], target_orientation[1], target_orientation[2]), target_orientation[3]))
        rigid_obj = env.task.tgt_obj
        rigid_obj_quaternion = rigid_obj.rotation
        ee_rot_mat = sim.robot.ee_T.rotation()
        ee_quaternion = mn.Quaternion.from_matrix(ee_rot_mat)
        # difference = ee_quaternion * rigid_obj_quaternion.conjugated()
        # TODO: compute difference between object angle and ee angle
        target_orientation_diff = target_orientation * rigid_obj_quaternion.conjugated()
        target_orientation_diff = np.array([target_orientation_diff.vector.x, target_orientation_diff.vector.y, target_orientation_diff.vector.z, target_orientation_diff.scalar])
        perturbation = 0.1
        perturbed_position = [target_position[i] + random.uniform(-perturbation, perturbation) for i in range(3)]
        # perturbed_position = [0.5, 0, 0.5]
        # target_position = env.task.place_goal - sim.robot.base_pos + np.asarray([0, 0, 0.05])
        arm_tgt_qpos = sim.pyb_robot.IK(target_position, target_orientation_diff, max_iters=10000)
        cur_qpos = np.array(robot.arm_joint_pos)
        ...
        # plan = np.ones((n_step-1, len(tgt_qpos))) * (tgt_qpos - cur_qpos) / (n_step - 1 + 1e-10) / 0.025
        print(target_position, ee_quaternion)
        return plan, arm_tgt_qpos

    def _place_rest(self, env):
        sim = env.sim
        robot = sim.robot
        target_position = np.asarray(sim.pyb_robot.ee_state[4])
        sim.pyb_robot.set_joint_states(sim.robot.params.arm_init_params)
        target_position = env.task.resting_position
        arm_tgt_qpos = sim.pyb_robot.IK(target_position, max_iters=10000)
        cur_qpos = np.array(robot.arm_joint_pos)
        tgt_qpos = np.array(arm_tgt_qpos)
        n_step = np.ceil(np.max(np.abs(tgt_qpos - cur_qpos)) / 0.01)
        n_step = max(1, int(n_step))
        ...
        while len(all_episode_stats) < num_eval_episodes:
            ...
            if done:
                ...
                if success_measure in info:
                    ...
                    episode_success = -1
                print(f"current_episode_reward = {current_episode_reward}, {episode_success}")
